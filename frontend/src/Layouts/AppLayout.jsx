    ...
    <>
      <div className="mb-8">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-slate-900">StockUp</h1>
            <p className="text-xs text-slate-500">Inventory command center</p>
          </div>
        </div>
      </div>
      <nav className="space-y-2" aria-label="Primary navigation">
        {navigation.map((item) => <NavLink key={item.href} item={item} active={isActive(item.href)} onClick={() => setOpen(false)} />)}
      </nav>
      <div className="mt-8 rounded-2xl bg-slate-50 p-4 text-sm text-slate-600">
        ...
        <p className="mt-1 text-xs">Manage stock, movement, finance, and reports in one place.</p>
      </div>
    </>
  );
    ...
    <div className="min-h-screen bg-slate-100 text-slate-900">
      ...
      <header className="sticky top-0 z-30 border-b border-slate-200 bg-white/95 px-4 py-4 backdrop-blur lg:hidden">
        <div className="flex items-center justify-between">
          <Link href="/" className="flex items-center gap-2 font-bold text-slate-900"><span className="flex h-9 w-9 items-center justify-center rounded-xl bg-emerald-600 text-white">S</span>StockUp</Link>
          <button type="button" onClick={() => setOpen(true)} className="rounded-xl border border-slate-300 px-3 py-2 text-sm font-semibold text-slate-700">Menu</button>
        </div>
      </header>

      {open ? <div className="fixed inset-0 z-40 bg-slate-900/40 lg:hidden" onClick={() => setOpen(false)} /> : null}
      <aside className={`fixed inset-y-0 left-0 z-50 w-80 max-w-[85vw] border-r border-slate-200 bg-white p-6 transition-transform lg:hidden ${open ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="mb-4 flex justify-end"><button type="button" onClick={() => setOpen(false)} className="rounded-lg px-2 py-1 text-sm text-slate-500 hover:bg-slate-100">Close</button></div>
        {sidebar}
      </aside>
